...
from .form import Profile_Form_for_admin_work
from .form import Profile2_form_for_admin_work
...
def user_profile(request):
    if request.method == 'POST':
        ...
        if pro2_form.is_valid() and pro_form.is_valid():
            ...
            messages.error(request, 'check the files size of format it should not exceed 1 mb')
            return redirect('profile')
...
def user_register(request):
    admn = staff.objects.get(emp_no = request.user.username)
    if request.method=='POST':
        ...
        if pass1==pass2:
            if User.objects.filter(email=email).exists():
                ...
                user_profile_obj.branch = admn.branch
