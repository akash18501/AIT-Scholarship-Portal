...
from .form import extra_documents_form

#changes
from .models import  current_session
obj = current_session.objects.get(id=1)
nowdate = obj.session
#changes
...
import datetime
import xlwt
# nowdate = datetime.datetime.today().year
...
def graphs():
    ...
    branchcount = {"ENTC": 7, "COMP": 6, "IT": 1, "MECH": 12}
...
def home(request):
    ...
    if request.user.is_authenticated:
        return render(request,'homepage/index.html',{'expired':expired,'not_expired':not_expired,"branch_count":branchcount,"dict":dict})
    else:
...
def delete_scholarship(request,x):
    ...
    try:
        ...
        return HttpResponse('ERROR in deleting scholarship not found')
...
def show_scholarship_template(request,x,y='0'):
    ...
    if not request.user.is_staff or request.user.is_superuser:
        ...
        return render(request, 'homepage/scholoarship_template.html',{'current_scholarship':current_scholarship,'p_form':form,'got_already':if_usergot_scholarship,"branch_count":branchcount,"dict":dict})
    else:
        applications=application_table.objects.all()
        admin=staff.objects.get(emp_no=request.user.username)
        var=admin.branch
...
def show_selected_applicants(request,x):
    ...
    try:
        all_admin_branches[0]
    except:
        all_admin_branches.append(0)

    try:
        all_admin_branches[1]
    except:
        all_admin_branches.append(0)

    try:
        all_admin_branches[2]
    except:
        all_admin_branches.append(0)

    try:
        all_admin_branches[3]
    except:
        all_admin_branches.append(0)
    ...
    if request.method=='POST':
        ...
        if request. user.profile.document10 and request.user.profile.document12 and request.user.profile.document_last_sem and request.user.profile.student_id and request.user.profile.father_id:
            ...
            if p_form.is_valid():
                ...
                messages.error(request, 'check the files size not exceeding 1 mb ')
